...
export function registerOfficialDocumentTool(server: McpServer) {
  server.tool(
    ...
    '학교 공문(기안문) 작성을 위한 가이드라인과 프롬프트를 반환합니다. 내부결재용 또는 외부기관 발송용 공문 초안을 작성하기 위한 구조화된 지침을 제공합니다. 결과물은 export_hwpx의 gonmun 템플릿으로 HWPX 파일로 출력할 수 있습니다.',
    {
    ...
    async (params) => {
      ...
      const guideMarkdown = [
        ...
        `## HWPX 출력 방법`,
        ``,
        `이 가이드에 따라 공문 내용을 작성한 후, \`export_hwpx\` 도구를 사용하여 HWPX 파일로 출력하세요.`,
        ``,
        `### export_hwpx 파라미터`,
        `- **template**: \`"gonmun"\``,
        `- **section_xml**: 아래 플레이스홀더에 작성한 내용을 채워서 전달`,
        ``,
        `### 플레이스홀더 매핑`,
        `| 플레이스홀더 | 내용 |`,
        `|-------------|------|`,
        ...
        ``,
        `### 주의사항`,
        `- section_xml에서 markdown 표를 사용하면 자동으로 OWPML hp:tbl로 변환됩니다.`,
        `- hp:linesegarray는 사용하지 마세요.`,
        `- hp:tbl 사용 시 반드시 rowCnt/colCnt를 포함하세요.`,
        ``,
        `## 예시`,
        ``,
        isExternal
