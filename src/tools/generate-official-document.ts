  ...
  server.tool(
    ...
    async (params) => {
      ...
      const guideMarkdown = [
        ...
        `   - 2항(주문): 구체적 요청/안내 사항`,
        `6. **붙임**: 첨부 파일이 있을 경우 "붙임  1. 세부 계획서 1부.  끝." 형식`,
        ``,
        `### 문체 규칙`,
        ...
        `## HWPX 문서 구성 방법`,
        ``,
        `공문은 \`gonmun\` 템플릿의 채워 넣을 칸에 필요한 내용을 넣어 만듭니다. 일반 사용자는 문서 내부 코드를 직접 작성하지 않아도 되며, 공문에 들어갈 실제 문구와 표 내용만 준비하면 됩니다.`,
        ``,
        `### 채워 넣을 칸 (gonmun 템플릿)`,
        `**공통 (내부/외부)**:`,
        `- {{기관명}}: ${schoolName}`,
        ...
        `- 공문 기안문 본문은 \`1.\`, \`가.\`, \`1)\`, \`가)\` 계층만 사용하고 \`□\`, \`○\`, \`-\`, \`※\` 같은 글머리기호를 쓰지 않습니다.`,
        `- 숫자·문자 계층과 글머리기호를 한 줄에 동시에 쓰지 않습니다. 예: \`1. □ 추진 개요\`, \`가. - 세부 내용\` 금지.`,
        `- \`-아래-\`, \`- 아 래 -\`, \`아래와 같이\` 구분 문구를 쓰지 않고 세부 내용은 다음 번호 항목이나 표로 바로 제시합니다.`,
        `- 붙임이 없으면 붙임 줄을 만들지 않고, 붙임이 있으면 \`붙임  1. 자료명 1부.  끝.\` 형식으로 정리합니다.`,
        ``,
        `## 예시`,
        ``,
        isExternal
          ? `> **제목**: 2026학년도 학교폭력 예방교육 실시 결과 보고\n> **본문1**: 관련: 미래교육지원청 학교폭력예방과-1234(2026. 4. 15.) "2026학년도 학교폭력 예방교육 실시 계획 안내"\n> **본문2**: 위 호와 관련하여 본교 학교폭력 예방교육 실시 결과를 보고하오니 업무에 참고하여 주시기 바랍니다.`
          : `> **제목**: 2026학년도 1학기 현장체험학습 실시 계획(안)\n> **본문1**: 본교 교육과정 운영 계획에 의거하여 학생들의 체험 중심 학습 기회를 제공하고자 합니다.\n> **본문2**: 현장체험학습 실시 계획을 검토하시고 승인하여 주시기 바랍니다.`,
      ].filter(Boolean).join('\n');
