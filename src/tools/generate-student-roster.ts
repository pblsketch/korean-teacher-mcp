  ...
  server.tool(
    ...
    async (params) => {
      ...
      const guideMarkdown = [
        ...
        `## HWPX 출력 안내`,
        ``,
        `명렬표는 **표(table)** 형식으로 작성하여 export_hwpx 도구로 출력합니다.`,
        `- 템플릿: "roster"`,
        `- 상단: 학급명 + 명렬표 제목 + 기간`,
        `- 중앙: 명렬표 본문 (markdown 표 → OWPML 자동 변환)`,
        `- 하단: 담임명, 작성일`,
        ``,
        `## 출력 구조`,
        ``,
        `1. **제목**: "${params.class_name || '○학년 ○반'} ${config.label}"`,
        `2. **기간**: ${params.date_range || '해당 기간'}`,
        `3. **명렬표**: ${config.columns.length}열 × ${studentList.length}행 표`,
        `4. **하단**: 담임명, 작성일, 기호 범례 (해당 시)`,
      ].filter(Boolean).join('\n');
