...
def validate_table_widths(work_dir: Path) -> None:
    """Check that no table width exceeds the printable area defined in section0.xml."""
    ...
    margin_match = re.search(r'<hp:pageMargin\b([^>]*)>', content)
    if not margin_match:
        return
    ...
    A4_WIDTH = 59528
    printable_width = A4_WIDTH - left - right

    # Find all <hp:sz width="X"> elements (table cell/row size definitions)
    for sz_match in re.finditer(r'<hp:sz\b[^>]*\bwidth="(\d+)"', content):
        width = int(sz_match.group(1))
        if width > printable_width:
            print(
                f"WARNING: Table width {width} exceeds printable width {printable_width} "
                f"(left={left}, right={right}) in section0.xml",
                file=sys.stderr,
            )
